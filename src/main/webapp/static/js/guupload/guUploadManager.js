...
var guUploadManager = function(option) {
	...
	var guFileList = null;
	var boxList= guTool.createElement("div", guupload, "guuploadBox");
...
//		guFileList = guTool.createElement("div", guupload, "guFileList", {height: "100%"});
		guFileList = guTool.createElement("div", boxList, "guFileList", {height: "100%"});
...
//		var swfbutton = guTool.createElement("span", guFileList);
		swfbutton.id="swfbutton";
	}
	this.form = option.form;

//	var scriptPath = getScriptURL()+"/js/guupload/";
	var scriptPath = getScriptURL();
	if (isrun_gu){
//		alert(scriptPath);
		...
		var btnMiddle= guTool.createElement("div", boxList,"middle",{width: "5px"});
		var btnLoc= guTool.createElement("div", boxList,"guBtn");
//		alert(btnLoc);
...
//		var browseBtn = guTool.createTextBox("button", guupload, "browseBtn", true);
		...
		var settings = {
				...
				file_size_limit : 1000*1024*1024, 		// 1000M  1G
				fileTag : filetag,
		...
		guFileList.innerHTML = '<div style="text-align: center;vertical-align: middle;">파일찾기버튼을 Click하세요</div>';
//		browseBtn=guFileList.createElement("div", guupload, "guBtn", {height: "40px"});
//		browseBtn=guTool.setStyle(src, styles);
