...
GUUpload.prototype.AddFile = function (file) {
	...
	if(this.fileCount++>this.settings.fileupCnt){
		
		if(document.getElementById("selectFile").value !=""){
			alert("파일을 1개만 업로그 가능합니다. 선택된 파일을 삭제하시기 바랍니다.");
			return;
		}
...
//	alert(document.getElementById("selectFile").value);
	...
	document.getElementById("selectFile").value=file.name;
...
GUUpload.prototype.uploadFiles = function () {
	for(var f in this.fileList) {
		...
		//alert(file.progressbar.className);

		var formData = new FormData();
		formData.append("Filedata", file.file, file.name);

		//alert(file[0]+"::::"+file.name);
